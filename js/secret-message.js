...
const setUserNav = () => {
  ...
  loginLink.textContent = currentUser ? "Profile" : "Login";
  loginLink.href = currentUser ? "profile.html" : "login.html";
};

const recordSecretSent = (senderEmail) => {
  if (!senderEmail) {
    return;
  }
  ...
  try {
    ...
    list.push({
      sender_email: String(senderEmail).toLowerCase(),
...
const fetchCloudMessages = async () => {
  const res = await fetch(
    ...
    { headers: supabaseHeaders(auth.getSession?.()?.access_token || "") }
  );
...
composerForm.addEventListener("submit", async (event) => {
  ...
  const currentUser = getCurrentUser();
  if (!currentUser) {
    composerNote.textContent = "Kamu harus login dulu sebelum kirim secret message.";
  ...
  try {
    ...
    recordSecretSent(currentUser.email);
